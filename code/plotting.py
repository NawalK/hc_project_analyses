...
class Plotting:
    ...
    def sc_plot(self, k_per_line=None, lthresh=2.3, uthresh=4.0, auto_thresh=False, perc_thresh=90, centering_method='max', show_spinal_levels=False, colormap='autumn', save_results=False):
        ...
        if save_results == True:
            if len(self.k.keys())==2:
                plt.savefig(self.config['output_dir'] + self.region + '_k_' + main_dataset + '_vs_' + secondary_dataset + ' _thr' + str(lthresh)+ 'to' + str(uthresh) + '.png')
            elif len(self.k.keys())==1:
                plt.savefig(self.config['output_dir'] + self.region + '_k_' + main_dataset + ' _thr' + str(lthresh)+ 'to' + str(uthresh) + '.png')
